...
async function openAddSessionSheet() {
  ...
  document.getElementById('ss-frequency').value = 'none';
  document.getElementById('ss-end-date').value = state.calSelectedDate || todayStr();
...
async function submitAddSession() {
  ...
  const frequency = document.getElementById('ss-frequency').value;
  const endDate = document.getElementById('ss-end-date').value || date;
...
function toggleRecurrenceFields() {
  const frequency = document.getElementById('ss-frequency')?.value;
  const wrap = document.getElementById('ss-end-date-wrap');
  if (wrap) wrap.style.display = frequency && frequency !== 'none' ? '' : 'none';
}
...
async function openSessionSheet(sessionId) {
  ...
  document.getElementById('sh-date').value = session.date;
  document.getElementById('sh-time').value = session.time;
  document.getElementById('sh-status').value = session.status;
  document.getElementById('sh-paid').checked = !!session.paid;
  document.getElementById('sh-save-series-btn').style.display = session.seriesId ? '' : 'none';
  document.getElementById('sh-delete-series-btn').style.display = session.seriesId ? '' : 'none';
...
async function saveSessionSeriesChanges() {
  if (!activeSessionCache?.seriesId) return;
  if (!confirm('Áp dụng trạng thái, giờ học và thanh toán cho toàn bộ chuỗi?')) return;
  try {
    await Store.updateSessionSeries(activeSessionCache.seriesId, {
      status: document.getElementById('sh-status').value,
      paid: document.getElementById('sh-paid').checked,
      time: document.getElementById('sh-time').value,
    });
    closeSessionSheet();
    refreshCurrentView();
    showToast('Đã cập nhật cả chuỗi lịch học');
...
async function deleteActiveSessionSeries() {
  if (!activeSessionCache?.seriesId) return;
  if (!confirm('Xóa toàn bộ các buổi trong chuỗi lịch học này?')) return;
  try {
    await Store.deleteSessionSeries(activeSessionCache.seriesId);
    closeSessionSheet();
    refreshCurrentView();
    showToast('Đã xóa cả chuỗi lịch học');
  } catch (e) { showToast(friendlyError(e)); }
}
...
async function fetchPaymentQrDataUrl(settings) {
  const url = window.SomaliSessionDomain.buildPaymentQrUrl(settings);
  if (!url) return null;
  try {
    ...
    console.warn('Không thể tải QR thanh toán', e);
    return null;
  }
}

async function drawPaymentBlock(doc, settings, startY) {
  const qrDataUrl = await fetchPaymentQrDataUrl(settings);
  const navy = [27, 58, 92];
  const gray = [90, 90, 90];
  doc.setDrawColor(225, 225, 225);
  doc.setFillColor(250, 250, 252);
  doc.roundedRect(14, startY, 182, 52, 3, 3, 'FD');
  if (qrDataUrl) {
    doc.addImage(qrDataUrl, 'PNG', 18, startY + 4, 44, 44);
  } else {
    doc.setFont('RobotoVN', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(...gray);
    doc.text('QR không khả dụng', 20, startY + 28);
  }
  doc.setFont('RobotoVN', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(...navy);
  doc.text('Thông tin thanh toán', 68, startY + 12);
  doc.setFont('RobotoVN', 'normal');
  doc.setFontSize(10);
  doc.setTextColor(...gray);
  doc.text(`Ngân hàng: ${settings.bank || '-'}`, 68, startY + 22);
  doc.text(`Số tài khoản: ${settings.accountNumber || '-'}`, 68, startY + 31);
  doc.text(`Chủ tài khoản: ${settings.accountName || '-'}`, 68, startY + 40);
  return startY + 60;
}
...
async function exportStudentPdfReport() {
  ...
  const [sessions, notes, balance, user, paymentSettings] = await Promise.all([
  ...
  // Payment block uses the latest saved bank data and is intentionally not the supplied full image.
  const sessionsStartY = await drawPaymentBlock(doc, paymentSettings, 84);
...
async function payAllStudentSessions() {
  if (!state.currentStudentId) return;
  try {
    const result = await Store.markStudentSessionsPaid(state.currentStudentId);
    if (!result.count) { showToast('Học sinh này không còn buổi nào chưa thanh toán'); return; }
    await openStudentDetail(state.currentStudentId);
    showToast(`Đã thanh toán ${result.count} buổi · ${fmtVND(result.total)}`);
  } catch (e) { showToast(friendlyError(e)); }
}
...
async function openSettingsSheet() {
  ...
  document.getElementById('settings-payment-bank').value = userSettingsCache.paymentBank || 'MB';
  document.getElementById('settings-payment-account').value = userSettingsCache.paymentAccountNumber || '0976406248';
  document.getElementById('settings-payment-name').value = userSettingsCache.paymentAccountName || 'NGUYEN THI THU HUONG';
...
async function savePaymentSettingsForm() {
  const bank = document.getElementById('settings-payment-bank').value.trim();
  const accountNumber = document.getElementById('settings-payment-account').value.trim();
  const accountName = document.getElementById('settings-payment-name').value.trim();
  if (!bank || !accountNumber || !accountName) { showToast('Vui lòng điền đủ thông tin thanh toán'); return; }
  try {
    await Store.savePaymentSettings({ bank, accountNumber, accountName });
    userSettingsCache.paymentBank = bank;
    userSettingsCache.paymentAccountNumber = accountNumber;
    userSettingsCache.paymentAccountName = accountName;
    showToast('Đã lưu thông tin thanh toán');
  } catch (e) { showToast(friendlyError(e)); }
}
