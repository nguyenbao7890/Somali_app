...
function mapSession(row) {
  return {
    ...
    seriesFrequency: row.series_frequency || 'none',
  };
}
...
const Store = {
  ...
  async addSession(input) {
    ...
    const row = {
      ...
      series_frequency: input.seriesFrequency || 'none',
    };
    const { data, error } = await supabaseClient.from('sessions').insert(row).select().single();
    if (error) throw error;
    return mapSession(data);
  },
  async addSessions(inputs) {
    if (!inputs.length) return [];
    const students = new Map();
    for (const input of inputs) {
      if (!students.has(input.studentId)) {
        const student = await this.getStudent(input.studentId);
        if (!student) throw new Error('Không tìm thấy học sinh để tạo buổi học.');
        students.set(input.studentId, student);
      }
    }
    const rowSeriesId = inputs.length > 1
      ? (inputs[0].seriesId || (typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random()}`))
      : (inputs[0].seriesId || null);
    const rows = inputs.map((input) => ({
      ...
      status: input.status ?? 'completed',
      paid: input.paid ?? true,
      fee_amount: students.get(input.studentId).feePerSession,
      note: input.note || '',
      series_id: rowSeriesId,
      series_frequency: input.seriesFrequency || 'none',
    }));
    const { data, error } = await supabaseClient.from('sessions').insert(rows).select();
    if (error) throw error;
    return data.map(mapSession);
  },
  async updateSession(id, patch) {
    ...
    if (patch.seriesFrequency !== undefined) row.series_frequency = patch.seriesFrequency;
  ...
  async updateSessionSeries(seriesId, patch) {
    const row = {};
    if (patch.status !== undefined) row.status = patch.status;
    if (patch.paid !== undefined) row.paid = patch.paid;
    if (patch.time !== undefined) row.time = patch.time;
    ...
    const { data, error } = await supabaseClient.from('sessions').update(row).eq('series_id', seriesId).select();
    if (error) throw error;
    return data.map(mapSession);
  },
  async deleteSessionSeries(seriesId) {
    const { error } = await supabaseClient.from('sessions').delete().eq('series_id', seriesId);
    if (error) throw error;
  },
  ...
  async getUserSettings() {
    ...
    if (error || !data) return this.defaultUserSettings();
    return {
      remindersEnabled: !!data.reminders_enabled,
      reminderMinutes: data.reminder_minutes || 30,
      paymentBank: data.payment_bank || 'MB',
      paymentAccountNumber: data.payment_account_number || '0976406248',
      paymentAccountName: data.payment_account_name || 'NGUYEN THI THU HUONG',
    };
  },
  defaultUserSettings() {
    return {
      ...
      reminderMinutes: 30,
      paymentBank: 'MB',
      paymentAccountNumber: '0976406248',
      paymentAccountName: 'NGUYEN THI THU HUONG',
    };
  },
  async saveUserSettings({ remindersEnabled, reminderMinutes, paymentBank, paymentAccountNumber, paymentAccountName }) {
    const row = {
      user_id: this.userId,
      reminders_enabled: remindersEnabled,
      reminder_minutes: reminderMinutes,
      payment_bank: paymentBank || 'MB',
      payment_account_number: paymentAccountNumber || '0976406248',
      payment_account_name: paymentAccountName || 'NGUYEN THI THU HUONG',
    };
  ...
  async savePaymentSettings({ bank, accountNumber, accountName }) {
  ...
  async markStudentSessionsPaid(studentId) {
    ...
    if (!sessions.length) return { count: 0, total: 0 };
    const { error } = await supabaseClient.from('sessions').update({ paid: true }).eq('student_id', studentId).eq('status', 'completed').eq('paid', false);
    if (error) throw error;
    return { count: sessions.length, total: sessions.reduce((sum, s) => sum + (s.feeAmount ?? student.feePerSession), 0) };
  },
