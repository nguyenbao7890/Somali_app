...
3. Vào **SQL Editor** → **New query** → copy toàn bộ nội dung file `supabase_schema.sql` → dán vào → nhấn **Run**. Bước này tạo sẵn các bảng dữ liệu và bảo mật theo tài khoản. Nếu đã chạy schema cũ, hãy chạy lại file để thêm cột `sessions.fee_amount`, giúp khóa đúng học phí tại thời điểm tạo buổi học.
...
- **Lịch học**: nội dung buổi học hiển thị trực tiếp trên từng ô ngày (giờ + tên học sinh, kiểu Google Calendar), thêm/sửa/xóa, đánh dấu hoàn thành/hủy/đã thanh toán
- **Học sinh**: danh sách, tìm kiếm, chi tiết từng học sinh gồm lịch sử buổi học, công nợ tự tính, nhận xét theo tháng
- **Báo cáo PDF**: nút "Xuất báo cáo PDF" trong trang chi tiết học sinh — xuất họ tên, SĐT, môn học, học phí, toàn bộ lịch sử buổi học và nhận xét từng tháng thành 1 file PDF đẹp, đầy đủ dấu tiếng Việt
...
supabase secrets set VAPID_PRIVATE_KEY=-6-akVpxBf5_y-QLXwxFRCBtIhtW3GqNi_d-Pi-oZ2c
supabase secrets set VAPID_SUBJECT=mailto:email-cua-ban@example.com
supabase secrets set CRON_SECRET=f2319cb4342e30c1dbaac43d589eab14531620162f1b4ec3
```
(2 khóa VAPID ở trên mình đã tạo sẵn cho bạn dùng luôn — không cần tự tạo. `CRON_SECRET` là một chuỗi bí mật tự đặt để chỉ pg_cron mới gọi được hàm gửi thông báo, bạn có thể giữ nguyên giá trị mẫu trên hoặc đổi thành chuỗi bất kỳ.)
